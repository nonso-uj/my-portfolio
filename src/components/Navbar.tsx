  ...
  return (
    <div className='fixed top-0 left-0 right-0 w-full h-fit z-50'>
      ...
      <div className='hidden w-full h-fit px-14 py-5 lg:flex flex-row items-center justify-between font-semibold text-xl duration-500'>
        <Link to={'/'}>
          <p className='uppercase font_bold navbar text-secondary'>Chinonso Udonne</p>
        </Link>
      ...
      <div className='lg:hidden w-full h-fit px-10 py-5 flex flex-row items-center justify-between backdrop-blur-sm bg-secondary-blur bg-white/10'>
        <Link to={'/'}>
          <p className='text-3xl font-bold font_bold navbar text-white'>C &middot; U</p>
        </Link>
        ...
        <div className="fixed top-4 right-5 w-fit h-fit block lg:hidden z-50">
          <Popover className="relative">
            {({ open }) => (
              <>
                <div className={`bg-white flex flex-col items-center justify-center w-12 h-12 rounded-full duration-300 ease-in-out ${ open && 'scale-125'}`}>
                ...
                <PopoverPanel anchor="bottom" className={`flex flex-col items-center justify-start w-full h-full p-3 pt-10  z-50`}>
                  ...
                  >
                    ...
                    <Link to={'/'}>
                        <p className={`${currentLocaton.pathname === "/" && 'text-[#D3E97A]'}`}>Home</p>
                    </Link>

                    <hr className="border border-white w-full" />
                    
                    <Link to={'/about'}>
                        <p className={`${currentLocaton.pathname === '/about' && 'text-[#D3E97A]'}`}>About</p>
                    </Link>

                    <hr className="border border-white w-full" />
                    
                    <Link to={'/projects'}>
                        <p className={`${currentLocaton.pathname === '/projects' && 'text-[#D3E97A]'}`}>Projects</p>
                    </Link>
